...
const AI_FILE_EXTENSIONS = new Set([
  ".json", ".jsonc", ".md", ".mjs", ".js", ".ts", ".toml", ".yaml", ".yml",
]);
...
export function isAiEvidencePath(filePath) {
  ...
  if (/^mcp(?:[._-].+)?\.(json|jsonc|toml|ya?ml|ts|js|mjs)$/.test(base)) return true;

  const aiDirectories = [
    ".claude", ".codex", "agents", "agent", "subagents", "prompts", "skills",
    "hooks", "evals", "evaluations", "guardrails", "mcp", "ai", "rag",
  ];
  if (aiDirectories.some((segment) => pathHasSegment(lower, segment))) {
    return true;
  }

  return /(^|\/)(ai|agentic|llm|rag|mcp)[-_].+\.(md|json|toml|ya?ml|ts|js|py)$/.test(lower);
}

export function isAiEvidenceCandidatePath(filePath) {
  if (!isAiEvidencePath(filePath)) return false;
  const extension = filePath.includes(".") ? `.${basename(filePath).split(".").at(-1)}` : "";
  return AI_FILE_BASENAMES.has(basename(filePath)) || AI_FILE_EXTENSIONS.has(extension);
}

export function aiEvidencePriority(filePath) {
  const lower = normalizedPath(filePath);
  const base = basename(lower);
  if (["claude.md", "agents.md", ".mcp.json", "mcp.json", "codex.toml"].includes(base)) return 100;
  if (pathHasSegment(lower, ".claude") || pathHasSegment(lower, ".codex")) return 90;
  if (pathHasSegment(lower, "mcp") || base.includes("mcp")) return 85;
  if (pathHasSegment(lower, "agents") || pathHasSegment(lower, "subagents")) return 80;
  if (pathHasSegment(lower, "hooks") || pathHasSegment(lower, "skills")) return 75;
  if (pathHasSegment(lower, "evals") || pathHasSegment(lower, "guardrails")) return 70;
  if (pathHasSegment(lower, "prompts")) return 65;
  return 50;
}
...
function analyseRepository(detail) {
  ...
  const content = manifests.map((item) => String(item.content ?? "").toLowerCase()).join("\n");
  ...
  const strongEvidence = mcpConfigs + agentFiles + hookFiles + evalFiles + guardrailFiles + Object.values(dependencies).filter(Boolean).length;
  const mediumEvidence = instructionFiles + promptFiles + skillFiles + moduleInstructionFiles + handoverFiles;
