# v21 data accuracy contract

## Public discovery metadata regression fix

GitHub-wide authored-PR and reviewed-PR searches intentionally return every
repository associated with a configured identity. That raw set can include:

- repositories owned by the profile account;
- public repositories owned by individual users rather than organizations;
- private or non-public repositories;
- empty repositories without a default branch;
- repository records that do not contain the metadata required by the
  external public-organization portfolio.

Those candidates are expected filters, not failed GitHub API requests. v21
resolves metadata into an explicit result containing either an eligible
normalized repository or a stable exclusion reason. Evidence is added through
an immutable, null-safe enrichment function. A filtered candidate can no longer
trigger `Cannot set properties of null (setting 'contribution_evidence')`.

The configured public-repository cap is now checked after eligibility
normalization, so irrelevant personal or user-owned search results do not
consume the external organization portfolio limit. Actual metadata request
failures still stop publication, preserving strict completeness.

`MAX_PUBLIC_CONTRIBUTION_SEARCH_CANDIDATES` separately limits raw metadata
inspection. Exceeding it fails the run with no truncation, protecting workflow
time and API quota while preserving correctness.

The offline data-pipeline self-test now covers both filtered-null and eligible
metadata candidates.
